...
import type { IndexerRequest, TestResult } from "@/api/indexers";
  ...
  return (
    <Modal onClose={onClose} width={520} innerStyle={{ padding: 24, gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={fieldStyle}>
              ...
              <input style={inputStyle} value={form.name} onChange={(e) => set("name", e.currentTarget.value)} onFocus={focusBorder} onBlur={blurBorder} placeholder="e.g. Prowlarr" autoFocus />
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Protocol</label>
              <select style={{ ...inputStyle, cursor: "pointer" }} value={form.kind} onChange={(e) => set("kind", e.currentTarget.value)} onFocus={focusBorder} onBlur={blurBorder}>
          ...
          <div style={{ background: "var(--color-bg-elevated)", border: "1px solid var(--color-border-subtle)", borderRadius: 8, padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--color-text-muted)" }}>{kindLabel} Settings</p>
            <div style={fieldStyle}>
              <label style={labelStyle}>URL *</label>
              <input style={inputStyle} value={form.url} onChange={(e) => set("url", e.currentTarget.value)} onFocus={focusBorder} onBlur={blurBorder} placeholder="https://indexer.example.com/torznab/api" />
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>API Key</label>
              <input style={inputStyle} type="password" value={form.api_key} onChange={(e) => set("api_key", e.currentTarget.value)} onFocus={focusBorder} onBlur={blurBorder} placeholder={editing ? "leave blank to keep existing key" : "optional"} autoComplete="new-password" />
              {editing && <p style={{ margin: "4px 0 0", fontSize: 11, color: "var(--color-text-muted)" }}>API keys are masked in the UI. Enter a new value to update.</p>}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={fieldStyle}>
              ...
              <input style={inputStyle} type="number" min="1" value={form.priority} onChange={(e) => set("priority", e.currentTarget.value)} onFocus={focusBorder} onBlur={blurBorder} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", paddingBottom: 2 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", userSelect: "none" }}>
                <input type="checkbox" checked={form.enabled} onChange={(e) => set("enabled", e.currentTarget.checked)} style={{ width: 16, height: 16, cursor: "pointer", accentColor: "var(--color-accent)" }} />
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button onClick={onClose} style={{ background: "none", border: "1px solid var(--color-border-default)", borderRadius: 6, padding: "8px 16px", fontSize: 13, color: "var(--color-text-secondary)", cursor: "pointer" }}
          ...
          >{isPending ? "Saving…" : editing ? "Save Changes" : "Add Indexer"}</button>
        </div>
    </Modal>
  );
}
...
function RowActions({ indexer, onEdit }: { indexer: IndexerConfig; onEdit: () => void }) {
  ...
  function handleTest() {
    ...
    test.mutate(indexer.id, {
      onSuccess: (r: TestResult) => { setTestResult(r); setTimeout(() => setTestResult(null), 4000); },
  ...
  if (confirming) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
        ...
        <button onClick={() => del.mutate(indexer.id, { onSuccess: () => setConfirming(false) })} disabled={del.isPending} style={actionBtn("var(--color-danger)", "color-mix(in srgb, var(--color-danger) 15%, transparent)")}>{del.isPending ? "…" : "Yes"}</button>
  ...
  if (testResult !== null) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
        <span style={{ fontSize: 12, color: testResult.ok ? "var(--color-success)" : "var(--color-danger)", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={testResult.message}>
...
function KindBadge({ kind }: { kind: string }) {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", background: isUsenet ? "color-mix(in srgb, var(--color-accent) 12%, transparent)" : "color-mix(in srgb, var(--color-success) 12%, transparent)", color: isUsenet ? "var(--color-accent)" : "var(--color-success)" }}>
      {kind}
    </span>
  );
}
...
export default function IndexerList() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing: IndexerConfig | null }>({ open: false, editing: null });
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900 }}>
      ...
      <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)", borderRadius: 8, boxShadow: "var(--shadow-card)", overflow: "hidden" }}>
        {isLoading ? (
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
            {[1, 2, 3].map((i) => (<div key={i} className="skeleton" style={{ height: 44, borderRadius: 4 }} />))}
          </div>
        ) : error ? (
          <div style={{ padding: 24, fontSize: 13, color: "var(--color-text-muted)" }}>Failed to load indexers.</div>
        ) : !data?.length ? (
          <div style={{ padding: 48, textAlign: "center" }}>
            <p style={{ margin: 0, fontSize: 14, color: "var(--color-text-secondary)", fontWeight: 500 }}>No indexers configured</p>
      ...
      {modal.open && <IndexerModal editing={modal.editing} onClose={() => setModal({ open: false, editing: null })} />}
    </div>
  );
}
