...
type updateConfigInput struct {
	Body struct {
		TVDBAPIKey string `json:"tvdb_api_key,omitempty" doc:"TVDB API key to persist"`
	}
}
...
func RegisterSystemRoutes(api huma.API, startTime time.Time, dbType, dbPath, configFile, apiKey string, logger *slog.Logger) {
	...
	huma.Register(api, huma.Operation{
		...
		if input.Body.TVDBAPIKey == "" {
			return nil, huma.NewError(http.StatusBadRequest, "no config values provided")
		}

		writePath, err := config.WriteConfigKey(configFile, "tvdb.api_key", input.Body.TVDBAPIKey)
		if err != nil {
			return nil, huma.NewError(http.StatusInternalServerError, "failed to write TVDB config", err)
		}
