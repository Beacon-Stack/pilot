...
//     Use for importers that legitimately connect to an internal host
//     (localhost or LAN) that the user controls.
package safedialer
...
// RFC-1918 and loopback are allowed because importers may need to reach
// an internal host (e.g. http://localhost:8989 or http://192.168.1.100:8989).
...
// private/loopback addresses (for internal service imports) while still
...
// user-configured internal services (e.g. a Sonarr instance on the LAN).
